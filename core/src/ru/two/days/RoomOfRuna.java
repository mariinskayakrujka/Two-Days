...
public class RoomOfRuna extends ScreenGame {
    ...
    String dairy3 = "Я поняла, что хочу любви. Достойна ли я ее? Мне хочется,\n чтобы кто-нибудь смотрел " +
            "на меня с таким счастьем в глазах, чтобы для кого-то\n я правда много значила, чтобы кто-то думал обо мне и улыбался. " +
            "Меня бы хотели обнять, поцеловать, пригреть у груди, сказать,\n что я хорошая, что им очень со мной повезло. " +
            "Однажды, когда-то\n давно, на курсе четвертом, ведь он повел себя со\n мной очень хорошо. " +
            ...
            "рассказывал о итогах революции. Я не сентиментальна.\n Он ушел, не сказав ни слова, а я еще где-то " +
            "минут десять плакала в\n пустой аудитории, пока не пришла другая группа.\n";
    String dairy4 = "29.15.\n" +
            "Я сходила впервые за этот семестр сходила на вордологию. Все смеялись,\n что я \"выделила место " +
            "в своем плотном графике и соизволила прийти\". Димен Стю\n не смеялся, но и не останавливал группу. " +
            "Всех заткнул только ректор, услышавший шум и\n зашедший к нам. Ему сказали, что такая шумиха из-за меня." +
            " А он ответил, что это все\n еще не повод смеяться. Я посидела еще минут пять\n после его ухода и убежала из аудитории.\n" +
            ...
            "ни слова не могу прочитать.\n Вало попыталась прочитать мне, но так еще хуже: я остановила.\n " +
    ...
    public RoomOfRuna(TwoDays myGG) {
        ...
        feli.add("Р: ...");}//добавление в список
        ...
        if (isReading) {
            if (isPaper) {
                ...
                if (countpaper != 5) {
                    ...
                    gg.batch.draw(letter, SCR_WIDTH / 2f - letter.getWidth() / 2f, SCR_HEIGHT / 2f - letter.getHeight() / 2f, SCR_HEIGHT, SCR_HEIGHT);
